...
class ServidorOlfato(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/":
            ...
            propias = [s for s in solicitudes if s["perro_id"] in mios]
            propias = [s for s in propias if quien["nombre"] not in s.get("archivado_por", [])]
            self.responder_json(propias)
            ...
            if not quien:
                self.responder_json({"error": "Sesión inválida"}, 403); return
            solicitudes = cargar(ARCHIVO_SOLICITUDES, [])
            mias = [s for s in solicitudes if s["adoptante"] == quien["nombre"]]
            mias = [s for s in mias if quien["nombre"] not in s.get("archivado_por", [])]
            self.responder_json(mias)
        ...
        # SSE: conexión en tiempo real para una conversación
            ...
            # Soporta token en header X-Token o en query ?token=...
            token_url = ""
            ruta = self.path
            if "?" in ruta:
                ...
                for par in qs.split("&"):
                    if par.startswith("token="):
                        token_url = par[6:]
            if token_url and not self.headers.get("X-Token"):
                self.headers = dict(self.headers)
                self.headers["X-Token"] = token_url
            quien = self.quien_soy()
            if not quien:
    ...
    def do_POST(self):
        ...
        # Enviar mensaje de chat
            ...
            perros.append({
                ...
                "estado": "disponible",
                "tipo": datos.get("tipo", "perro"),
                "edad_raw": datos.get("edad_raw"),
                "edad_unidad": datos.get("edad_unidad", "anios"),
        ...
        # Ocultar una solicitud/chat de la vista de un usuario (sin borrar nada)
        elif self.path == "/api/solicitudes/archivar":
            datos = self.leer_cuerpo()
            quien = self.quien_soy()
            if not quien:
                self.responder_json({"error": "Sesión inválida"}, 403); return
            solicitudes = cargar(ARCHIVO_SOLICITUDES, [])
            for s in solicitudes:
                if s["id"] == datos["solicitud_id"]:
                    archivado_por = s.get("archivado_por", [])
                    if quien["nombre"] not in archivado_por:
                        archivado_por.append(quien["nombre"])
                    s["archivado_por"] = archivado_por
            guardar(ARCHIVO_SOLICITUDES, solicitudes)
            self.responder_json({"ok": True})
